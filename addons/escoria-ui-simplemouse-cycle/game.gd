...
"""
Simple Mouse Cycle UI

- Left click executes the current verb
- Right click cycles verbs (walk / look / pickup / use / talk)
- Mousewheel and gamepad Y also cycle verbs
- Hover auto-switches verbs like Simple Mouse; leaving a hotspot
  clears a manual cycle so contextual verbs apply again
"""

const VERB_USE = "use"
const VERB_WALK = "walk"

# System/framework events that should not put the UI into "busy"/hourglass mode.
...
# True while a gameplay event is running (hourglass / blocked world input).
var _interaction_busy: bool = false

# True when this UI set INPUT_SKIP and must restore INPUT_ALL afterwards.
var _ui_owns_input_lock: bool = false

# True while a multi-answer dialog chooser is visible.
var _awaiting_dialog_choice: bool = false
...
func _ready():
	...
	escoria.event_manager.event_started.connect(_on_gameplay_event_started)
	call_deferred("_connect_dialog_player_signals")


func _connect_dialog_player_signals() -> void:
	var dialog_player = escoria.dialog_player
	if dialog_player == null:
		return
	if not dialog_player.choices_started.is_connected(_on_dialog_choices_started):
		dialog_player.choices_started.connect(_on_dialog_choices_started)
	if not dialog_player.option_chosen.is_connected(_on_dialog_option_chosen):
		dialog_player.option_chosen.connect(_on_dialog_option_chosen)
...
func _process_input(event: InputEvent, is_default_state: bool) -> bool:
	...
	if _interaction_busy:
		# World actions are blocked; dialog skip is handled by the dialog state machine.
		return false
	...
	# Action is only registered while a gamepad is connected.
	if InputMap.has_action(ESC_UI_CHANGE_VERB_ACTION) \
			and event.is_action_pressed(ESC_UI_CHANGE_VERB_ACTION):
		mousewheel_action(1)
		return true
	return false


func _cycle_verb() -> void:
	if _interaction_busy:
		return
	$mouse_layer/verbs_menu.iterate_actions_cursor(1)


func _on_gameplay_event_started(event_name: String) -> void:
	if event_name in SYSTEM_EVENTS:
		return
	_enter_busy_mode()


func _on_dialog_choices_started() -> void:
	# Classical UI: hourglass during speech, normal verb cursor while picking answers.
	_awaiting_dialog_choice = true
	$ui/tooltip.clear()
	$mouse_layer/verbs_menu.set_dialog_choice_cursor()


func _on_dialog_option_chosen(_option) -> void:
	if not _awaiting_dialog_choice:
		return
	_awaiting_dialog_choice = false
	# If the scripted event continues after the choice, restore the hourglass.
	if _interaction_busy:
		$ui/tooltip.clear()
		$mouse_layer/verbs_menu.set_by_name("wait")


func _enter_busy_mode() -> void:
	...
	# Allow dialog skip/finish clicks, but block other world interactions.
	if escoria.inputs_manager.input_mode == escoria.inputs_manager.INPUT_ALL:
		_ui_owns_input_lock = true
		escoria.inputs_manager.input_mode = escoria.inputs_manager.INPUT_SKIP
	else:
		_ui_owns_input_lock = false


func _exit_busy_mode() -> void:
	if not _interaction_busy:
		return

	_awaiting_dialog_choice = false

	if _ui_owns_input_lock:
		_ui_owns_input_lock = false
		_interaction_busy = false
		if escoria.inputs_manager.input_mode == escoria.inputs_manager.INPUT_SKIP:
			escoria.inputs_manager.input_mode = escoria.inputs_manager.INPUT_ALL
		_refresh_cursor_from_hover()
		return

	# Input was locked by a script/cutscene; wait until INPUT_ALL before snapping.
	if escoria.inputs_manager.input_mode == escoria.inputs_manager.INPUT_ALL:
		_interaction_busy = false
		_refresh_cursor_from_hover()


func _refresh_cursor_from_hover() -> void:
	$mouse_layer/verbs_menu.action_manually_changed = false
	escoria.action_manager.clear_current_tool()
	$mouse_layer/verbs_menu.clear_tool_texture()
	$ui/tooltip.clear()

	var focused = escoria.inputs_manager.hotspot_focused
	if focused.is_empty():
...
func left_click_on_bg(position: Vector2) -> void:
	...
	if escoria.main.current_scene.player:
		...
		# Keep the selected verb after walking; only clear an attached tool.
		$mouse_layer/verbs_menu.clear_tool_texture()
	close_inventory()

func right_click_on_bg(_position: Vector2) -> void:
	_cycle_verb()
...
func element_focused(element_id: String) -> void:
	# While busy, keep the hourglass and never leave a sticky tooltip behind.
	if _interaction_busy:
		$ui/tooltip.clear()
		return
	...
	if escoria.action_manager.current_action != VERB_USE \
			...
			if target_obj.is_exit:
				...
				var default_action = target_obj.default_action
				if default_action.is_empty():
					default_action = VERB_WALK
				$mouse_layer/verbs_menu.set_by_name(default_action)

func element_unfocused() -> void:
	$ui/tooltip.clear()
	if _interaction_busy:
		return
	# Leaving a hotspot restores contextual hover behaviour (e.g. walk on ground).
...
func right_click_on_item(_item_global_id: String, _event: InputEvent) -> void:
	_cycle_verb()
...
func inventory_item_focused(inventory_item_global_id: String) -> void:
	...
	if not $mouse_layer/verbs_menu.action_manually_changed:
		...
		var default_action = item_node.default_action_inventory
		if default_action.is_empty():
			default_action = VERB_WALK
		$mouse_layer/verbs_menu.set_by_name(default_action)
...
func _on_action_finished():
	$mouse_layer/verbs_menu.clear_tool_texture()
	# If a gameplay event is still running, keep the hourglass
	# (unless the player is currently picking a dialog answer).
	if _interaction_busy:
		$ui/tooltip.clear()
		if _awaiting_dialog_choice:
			$mouse_layer/verbs_menu.set_dialog_choice_cursor()
		else:
			$mouse_layer/verbs_menu.set_by_name("wait")
		return
...
func _on_event_done(_return_code: int, _event_name: String):
	if _return_code == ESCExecution.RC_OK or _return_code == ESCExecution.RC_CANCEL:
		_exit_busy_mode()
...
func _on_input_mode_changed(new_mode):
	if new_mode == ESCInputsManager.INPUT_NONE \
			or new_mode == ESCInputsManager.INPUT_SKIP:
		_interaction_busy = true
		$ui/tooltip.clear()
		if _awaiting_dialog_choice and new_mode == ESCInputsManager.INPUT_SKIP:
			$mouse_layer/verbs_menu.set_dialog_choice_cursor()
		else:
			$mouse_layer/verbs_menu.set_by_name("wait")
		return
	if (
		...
		return
	# Inputs re-enabled: snap to whatever is under the cursor now.
	_interaction_busy = false
	_awaiting_dialog_choice = false
	_refresh_cursor_from_hover()
