## An abstraction class to expose the same animation methods for both[br]
## AnimatedSprite and AnimationPlayer
extends Node
class_name ESCAnimationPlayer


## Signal emitted when the animation finsihed playing
...
## Return true if the ESCAnimationPlayer node is valid, ie. it has a valid player[br]
## node.[br]
